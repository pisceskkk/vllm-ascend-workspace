...
- Prefer incremental Git `receive-pack` directly into the container-local bare
  mirrors. In `auto` mode, fall back to the acknowledged sub-MTU full-bundle
  transfer when Git push is unavailable; verify fallback bundle SHA256 before
  importing it.
- Runtime install progress should be attributable at the package-step level: uninstall, vLLM Rust build requirements, `vllm`, `vllm-ascend` requirements, `vllm-ascend`, import smoke, and marker write.
...
- Runtime editable installs use `--no-deps`; dependency ownership stays in `vllm-ascend` requirements. The only vLLM-side bootstrap is its upstream-declared `requirements/build/rust.txt`; never install the CUDA build requirements because they may replace the image's paired Torch stack. Cache / compile env such as `PIP_CACHE_DIR`, `FETCHCONTENT_BASE_DIR`, `VAWS_BUILD_JOBS`, `MAX_JOBS`, and `CMAKE_BUILD_PARALLEL_LEVEL` may be explicitly passed into the remote install shell, and the effective remote install env is recorded in the manifest/runtime state with URL userinfo redacted.
- If editable install, public-API import verification, or dependency verification fails, fail closed with the captured log instead of trying alternate package sources, installing `uv`, refreshing the packaging stack, or changing install modes. Run import verification outside the workspace root so the outer `vllm/` repository cannot masquerade as the installed package.
...
- in `auto` mode, if `git-receive-pack` cannot complete, create a self-contained
  local Git bundle, stream it through the acknowledged framed transport, and
  fetch it into the same mirror
