...
- under Codex, run the entire parity entrypoint in the approved host execution
  plane; a sandbox overflow UID/GID view returns
  `ssh_host_execution_required` before OpenSSH or remote mutation
...
- transfer manifests and fallback Git bundles through acknowledged sub-MTU
  frames and require the remote SHA256 to match before mirror import
